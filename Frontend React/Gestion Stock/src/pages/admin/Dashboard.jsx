...
export function AdminDashboard() {
  ...
  const ventasHoy = useMemo(() => {
    ...
    return movimientos.filter((m) => m.tipo === 'SALIDA' && String(m.fecha).slice(0, 10) === k).length
  }, [movimientos])

  const movimientosHoy = useMemo(() => {
    const k = todayKeyLocal()
    return movimientos.filter((m) => String(m.fecha).slice(0, 10) === k).length
  }, [movimientos])

  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <StatCard title="Total productos" value={new Intl.NumberFormat('es-CL').format(totalProductos)} delta={0} icon={Boxes} accent="red" />
        <StatCard title="Stock critico" value={new Intl.NumberFormat('es-CL').format(criticos)} delta={criticos ? -4 : 0} icon={TriangleAlert} accent="red" />
        <StatCard title="Ventas del dia" value={new Intl.NumberFormat('es-CL').format(ventasHoy)} delta={12} icon={ShoppingCart} accent="green" />
      </div>
          ...
          <CardBody>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <Link to="/admin/historial/nuevo" className="block">
                <div className="rounded-2xl border border-[rgba(255,255,255,0.06)] bg-white/3 px-4 py-4 hover:bg-white/5 transition">
                  <div className="flex items-center justify-between gap-3">
                    <div className="flex items-center gap-2 text-zinc-100 font-semibold"><History size={16} /> Crear historial</div>
                    <ArrowRight size={16} className="text-zinc-400" />
                  </div>
        ...
        <Card>
          ...
          <CardBody>
            <div className="space-y-3">
              ...
              <Link to="/admin/historial/nuevo" className="block">
                <Button variant="primary" className="w-full justify-center">Crear movimiento</Button>
              </Link>
            </div>
          </CardBody>
        </Card>
