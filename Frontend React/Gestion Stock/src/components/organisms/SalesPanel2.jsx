...
export function SalesPanel2() {
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return productosMapped
    return productosMapped.filter((p) => `${p.sku} ${p.name}`.toLowerCase().includes(q))
  }, [productosMapped, query])
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[1.2fr_0.8fr] gap-4">
      <Card>
        ...
        <CardBody>
          ...
          <div className="pt-4 text-xs text-[var(--muted)] flex items-center gap-2">
            <ScanLine size={14} />
            Tip: pega un codigo (scanner) o clic para agregar.
          </div>
        </CardBody>
      </Card>
      ...
      <Modal open={confirmOpen} onClose={() => setConfirmOpen(false)} title="Registrar salida (demo)">
        <div className="space-y-3">
          ...
          <Subtle>
            Simulacion: descuenta stock y agrega un movimiento. El objetivo es UX y fluidez.
          </Subtle>
