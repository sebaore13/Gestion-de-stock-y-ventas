...
function buildTicket(sale, origen) {
  const lines = []
  ...
  lines.push(
    padRight('N°celular: +569 9378 3219', W)
  )

  lines.push(
    padRight('Direccion: Correa 277b, Melipilla', W)
  )
  ...
  if (sale.items && sale.items.length > 0) {
    ...
    for (const item of sale.items) {
      ...
      const total =
        '$' + formatear(item.precio * item.cantidad)

      const line1 =
        padRight(nombre.slice(0, W - 17), W - 17) +
        padLeft(cant, 5) +
        '  ' +
        padLeft(total, 10)
  ...
  lines.push(
    padRight('TOTAL', W - 12) +
    padLeft('$' + formatear(sale.total), 12)
  )
  ...
  empty()
  lines.push(
    padCenter('Gracias por su compra', W)
  )
  ...
  const textBuf = linesToEscpos(
    lines,
    bigHeader('PitstopPRO')
  )

  // Venta normal:
  // imprime + corta papel + abre cajón
  //
  // Reimpresión:
  // imprime + corta papel, pero NO abre cajón
  return origen === 'reprint'
    ? textBuf
    : Buffer.concat([
        textBuf,
        Buffer.from([
...
function linesToEscpos(lines, prefix) {
  ...
  for (const line of lines) {
    // Si es una línea de separación con = o -
    if (
      line.startsWith('=') ||
      line.startsWith('-')
    ) {
...
function buildQuotation(q) {
  ...
  lines.push(
    padCenter(formatFecha(q.fecha), W)
  )
  ...
  lines.push(
    padRight(
      'Direccion: Correa 277b, Melipilla',
      W
    )
  )
...
// Envía buffer raw ESC/POS a la impresora
// via cmd copy /b (Windows)
function printBuffer(buffer, printerName) {
  return new Promise((resolve, reject) => {
    const tmpFile = path.join(
      __dirname,
      `__print_${Date.now()}.prn`
    )
      ...
      (err) => {
        ...
        if (err) {
          return reject(
            new Error(
              `No se pudo imprimir. ` +
              `Asegurate de compartir la impresora:\n` +
